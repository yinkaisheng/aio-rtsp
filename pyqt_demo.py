...
class RtspPlayerThread(QtCore.QThread):
    ...
    def _play(self) -> None:
        ...
        def network_thread() -> None:
            try:
                asyncio.run(self._aplay_with_queue(msg_queue))
            except Exception as ex:
                msg_queue.put(ex)
                self.stop_event.set()
            ...
            while True:
                ...
                if isinstance(event, ConnectResultEvent):
                    ...
                    if event.method == "DESCRIBE" and event.status_code == 200:
                        ...
                        codec_name_lower = video_sdp.get("codec_name", "").lower()
                        if video_sdp:
                            self.log_ready.emit(
                                f"{Tick.process_tick()} video stream: codec={video_sdp.get('codec_name', 'unknown')}, clock_rate={video_clock_rate}"
                            )
                        if codec_name_lower:
                            av_codec_name = (
                                aiortsp.HEVCCodecName if codec_name_lower == aiortsp.H265CodecName else codec_name_lower
                            )
                        ...
                        if audio_sdp:
                            self.log_ready.emit(
                                f"{Tick.process_tick()} audio stream: codec={audio_sdp.get('codec_name', 'unknown')}, sample_rate={audio_sdp.get('clock_rate', 0)}, channels={audio_sdp.get('channel', 1)}"
                            )
                    ...
                    if not got_i_frame:
                        if codec_name_lower == aiortsp.H264CodecName and not (5 <= vframe.nalu_type <= 8):
                            continue
                        if codec_name_lower == aiortsp.H265CodecName and not (16 <= vframe.nalu_type <= 40):
                            continue
                        if is_corrupt_frame.get(vframe.timestamp, False):
                            self.log_ready.emit(f"{Tick.process_tick()} skip corrupt raw frame ts={vframe.timestamp}")
                            continue
                    ...
                    for packet in packets:
                        ...
                        try:
                            ...
                            self.log_ready.emit(f"{Tick.process_tick()} decode error pts={packet.pts}, ex={ex!r}")
                            continue
    ...
    async def _aplay_with_queue(self, msg_queue: queue.Queue) -> None:
